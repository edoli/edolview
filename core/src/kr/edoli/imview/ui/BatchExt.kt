...
fun Batch.drawRoundRect(x: Float, y: Float, width: Float, height: Float, radius: Float) {
    idx = 0
    index = 0
    ...
    (this as PolygonSpriteBatch).draw(Textures.white, vertices, 0, idx, indicies, 0, index)
}

fun Batch.drawCircle(x: Float, y: Float, radius: Float) {
    drawArc(x, y, radius, 0f, 360f)
}

fun Batch.drawArc(x: Float, y: Float, radius: Float, fromAngle: Float, toAngle: Float) {
    idx = 0
    index = 0
